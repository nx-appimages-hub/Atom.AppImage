...
all:
	...
	wget --output-document="$(PWD)/build/build.deb" "https://atom-installer.github.com/v1.41.0/atom-amd64.deb?s=1571754160&ext=.deb"
	...
	mkdir --parents $(PWD)/build/AppDir/atom	
	cp --force --recursive $(PWD)/build/usr/* $(PWD)/build/AppDir/
	cp --force --recursive $(PWD)/build/usr/share/atom/* $(PWD)/build/AppDir/atom
	cp --force --recursive $(PWD)/AppDir/* $(PWD)/build/AppDir

	rm -rf $(PWD)/AppDir/opt
	rm -rf $(PWD)/build/usr/share/atom
